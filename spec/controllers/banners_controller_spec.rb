...
require_dependency 'banners_controller'
class BannersController < ApplicationController; def rescue_action(e); raise e; end end

describe BannersController do
  # Uses make_resourceful, which has its own specs. Only testing customizations
  scenario :users
  before :each do
  ...
  [:admin, :developer].each do |user|
    describe "#{user} user" do
      ...
      def redirects_to_index
        ...
        response.should redirect_to(banners_path)
      end
